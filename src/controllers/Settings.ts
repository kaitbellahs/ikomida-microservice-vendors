...
export default class Settings {
    ...
    try {
      ...
      if (payload?.mainPicture?.includes('data:')) {
        ...
        try {
          ...
          new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_UPDATE_PROFILE_EXCEPTION, exception).log(this.logger);
        }
      }
  ...
  async integratePagseguroGateway(identity: Types.Classes.CUser, input: any) {
    try {
      const payload: Types.Classes.CvendorPagseguroIntegration = Types.Classes.CvendorPagseguroIntegration.fromObject(input);
      if (!payload.validate()) {
      ...
      if (!contractModel || contractModel.ikomidaID !== payload?.state) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_INTEGRATE_PAGSEGURO_INVALID_CONTRACT);
        return error.logAndReturn(this.logger);
      }
      const vendorSettingsModel = contractModel?.vendorSettings;
      if (!vendorSettingsModel) {
      ...
      if (!response) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_INTEGRATE_PAGSEGURO_EMPTY_RESPONSE);
        return error.logAndReturn(this.logger);
      }
  ...
  async revokePagseguroGateway(identity: Types.Classes.CUser) {
    try {
      ...
      if (!contractModel) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_REVOKE_PAGSEGURO_INVALID_CONTRACT);
        return error.logAndReturn(this.logger);
      }
      const vendorSettingsModel = contractModel?.vendorSettings;
      if (!vendorSettingsModel) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_REVOKE_PAGSEGURO_INVALID_VENDOR_SETTINGS);
        return error.logAndReturn(this.logger);
      }
      const vendorPaymentGatewayModel = vendorSettingsModel?.vendorPaymentGateway;
      if (vendorPaymentGatewayModel) {
      ...
      const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_SET_REVOKE_PAGSEGURO_EXCEPTION, exception);
      return error.logAndReturn(this.logger);
    }
  }
  ...
  async updateBusinessHours(identity: Types.Classes.CUser, object: any) {
    try {
      ...
      if (!businessTime.validate()) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_UPDATE_BUSNIESS_HOURS_MISSING_DATA);
        return error.logAndReturn(this.logger);
      }
      const contractModel = await DBModels.ContractModel.findOne({
        where: {
      ...
      if (!contractModel) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_UPDATE_BUSNIESS_HOURS_INVALID_CONTRACT);
        return error.logAndReturn(this.logger);
      }
      const vendorSettingsModel = contractModel?.vendorSettings;
      if (!vendorSettingsModel) {
      ...
      const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_UPDATE_BUSNIESS_HOURS_EXCEPTION, exception);
      return error.logAndReturn(this.logger);
    }
  }
  ...
  async updateDelivery(identity: Types.Classes.CUser, object: any) {
    try {
      ...
      const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_VENDOR_SERVICE_UPDATE_DELIVERY_EXCEPTION, exception);
      return error.logAndReturn(this.logger);
    }
  }
      ...
      const contractModel = await DBModels.ContractModel.findOne({
        ...
        include: [
          ...
          },
          {
            ...
            where: {
              createdAt: {
                [Domain.SqlDB.Op.gte]: Domain.SqlDB.Column('contractPaymentSignature.lastDueDate'),
              },
              status: {
